...
- Current catalog size: 25 lineups. Own captures (`captureCredit: PRÓPRIA`) cover Mirage smokes L/CT/Cabecinha/Jungle CT/Janela B, Mirage Flash Meio, Dust2 Portas do meio (replaced CS2Nades composite; source kept), Portas do meio v2, Domínio Fundo, and Porta B, plus Flash on Inferno/Nuke/Ancient/Cache and one Anubis molotov. Capture clicks open an in-page lightbox with wheel zoom and drag-to-pan; a lineup can include multiple labeled captures. Local-only `source` values (no `http`) keep favorites working and show `ID · …` in the card footer instead of an external video link.
